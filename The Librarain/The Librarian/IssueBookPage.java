...
public class IssueBookPage extends javax.swing.JFrame {
    ...
    private void FillComboBox() {
        ...
        Statement stb, stm;
        ResultSet rsb, rsm;
        String query_b = "SELECT * FROM `books`";
        String query_m = "SELECT * FROM `member`";

        try {
            stb = DBConnection.getConnection().createStatement();
            rsb = stb.executeQuery(query_b);
            while (rsb.next()) {
                jCB_BookID.addItem(rsb.getString("id") + "  " + rsb.getString("name"));
            }

            stm = DBConnection.getConnection().createStatement();
            rsm = stm.executeQuery(query_m);
            while (rsm.next()) {
                jCB_MemID.addItem(rsm.getString("id"));
            }
    ...
    public boolean checkIssueBook() {

        Statement st;
        ResultSet rs;
        boolean checkIssueBook = false;
        ...
        String IssueDate = dcn.format(jDC_IssDate.getDate());
        String ReturnDate = dcn.format(jDC_ReDate.getDate());

        PreparedStatement ps;
        String sql = "INSERT INTO `issued_book`(`book_id`, `mem_id`, `mem_name`, `issue_date`, `return_date`, `status`) VALUES (?,?,?,?,?,?)";

        try {
            st = DBConnection.getConnection().createStatement();
            rs = st.executeQuery(query + Memid);

            ps = DBConnection.getConnection().prepareStatement(sql);
            ps.setString(1, Bookid);
            ps.setString(2, Memid);
            if (rs.next()) {
                ps.setString(3, rs.getString("fname") + " " + rs.getString("lname"));
            }
            ps.setString(4, IssueDate);
            ps.setString(5, ReturnDate);
            ps.setString(6, "pending");

            checkIssueBook = ps.executeUpdate() > 0;
            
        } catch (SQLException ex) {
            System.out.println(ex);
        }
        return checkIssueBook;
    }
    ...
    public boolean isAlreadyIssued() {
        ...
        PreparedStatement ps;
        ResultSet rs;
        String query = "SELECT * FROM `issued_book` WHERE book_id=? AND status=?";
    ...
    private void initComponents() {
        ...
        jDC_IssDate.setDateFormatString("dd-MM-yyyy");
        ...
        jDC_ReDate.setDateFormatString("dd-MM-yyyy");
        ...
        H_IssueBook.setText("Issue Book");
